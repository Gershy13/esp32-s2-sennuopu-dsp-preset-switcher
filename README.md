...
A WiFi-controlled preset switcher for the Sennuopu DSPs (), running on an ESP32-S2.
The ESP32-S2 acts as a USB host, communicates with the DSP over HID control transfers,
and exposes a browser-based UI over WebSockets so presets can be changed from any device
on the same network.
...
2. It starts a USB host and waits for the DSP to be plugged in via USB.
3. An HTTP server and WebSocket endpoint (`/ws`) are started; the web UI is served from LittleFS.
4. Opening the IP address in a browser shows six preset buttons and the current connection status.
...
The USB host functionality requires a fork of the EspTinyUSB library that extends it with USB host support. Install it manually:
...
The ESP32 Arduino core (espressif/arduino-esp32) provides `WiFi`, `LittleFS`, and the underlying USB host headers that the library builds on.

### 3. Board settings

| Setting | Value |
|---|---|
| Board | ESP32S2 Dev Module (or your specific S2 board) |
| USB Mode | USB-OTG (TinyUSB) |
| Partition Scheme | Default 4MB with spiffs / any scheme with a data partition |
...
All device-specific values are near the top of the `.ino`:
